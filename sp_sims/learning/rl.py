...
class ReplayBuffer:
    ...
    def __init__(self, 
        ...
        self.samprate_slowest = samprate_slowest
        self.memory = deque(maxlen=buffer_size)

        # Calculate Furthest Sample
        self.furthest_sample =  self.samprate_slowest*(self.sampbudget-1)# Remember that init state is fixed
...
def populate_replay_buffer(rbuffer,num_of_examples, sample_budget, guesses_per_rate = 1000):
    ...
    # For Every Generator Rate example
    for i in range(num_of_examples):# For Every State-Action
        ...
        for j in range(guesses_per_rate): # Sample a bunch of paths
            roe = RaceOfExponentials(sample_budget,rates[true_hyps[j]][i],state_limit=1)
            ...
            tmpSampTape, replicas = quick_sample_budget(smp_rates[i], state_tape,holdTimes_tape, budget=sample_budget)
