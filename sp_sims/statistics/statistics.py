...
def quick_sample(sampling_rate,state_tapes,holding_t_tapes, max_samples = np.inf):
    ...
    np_state_array = np.asarray(state_tapes)
    currT = 0
    currStatIdx = 0
    
    while currT < transition_times[-1] and len(states) < max_samples:
        #  print(str(currT) + ' out of ' + str(transition_times[-1]))
        if currStatIdx >= len(transition_times):
            break
        
        # Index This is tryign to find how many transitions we can fit into a step.
        idxIncrement = 1
        ...
        # How many samples we can fit into our observation. i-> j
        NoOfReplica = int(np.floor(
            (transition_times[currStatIdx + idxIncrement - 1] - currT) 
                / sampling_time
            ))
        

        states = states + ([np_state_array[currStatIdx + idxIncrement - 1]] * NoOfReplica)
        # Set Time after all the replicas 
